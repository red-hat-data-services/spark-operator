...
- [ ] `deployment.yaml` - check for:
  ...
  - [ ] **NO fsGroup** in securityContext
  - [ ] Updated image tag
- [ ] `manifests.yaml` - **auto-generated by `make manifests`**, contains combined MutatingWebhookConfiguration and ValidatingWebhookConfiguration. This is the file referenced by `kustomization.yaml` and deployed to the cluster. Webhook names (`mutating-webhook-configuration` / `validating-webhook-configuration`) match upstream.
...
- [ ] `rolebinding.yaml` - usually unchanged
- [ ] `spark-application-rbac.yaml` - SA, Role, and RoleBinding for Spark driver pods (check for new permissions)
...
│   ├── spark-application-rbac.yaml   # SA, Role, RoleBinding for Spark driver pods
...
└── webhook/
    ...
    ├── kustomization.yaml
    ├── kustomizeconfig.yaml
    ├── manifests.yaml                # Auto-generated, used by kustomize
    ├── role.yaml
    ├── rolebinding.yaml
    ├── service.yaml
    ├── serviceaccount.yaml
    ├── webhook-objectselector-patch.yaml
    └── webhook-validating-selector-patch.yaml
```
